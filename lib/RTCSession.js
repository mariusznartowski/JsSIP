...
RTCSession.setCameraResolutionInSIP = function(description) {
	/* MN poprawki związane z rozdzielczością kamery */
	console.log('MN RTCSession.setCameraResolutionInSIP');
	console.log(description.sdp);
	description.sdp = description.sdp.replace( /b=AS([^\r\n]+\r\n)/g , '');
	//description.sdp = description.sdp.replace( /a=mid:video\r\n/g , 'a=mid:video\r\nb=AS:1024\r\na=fmtp:100 maxaveragebitrate=1024000; minaveragebitrate=1024000; x-google-min-bitrate=1024; x-google-max-bitrate=1024;\r\n');
	description.sdp = description.sdp.replace( /a=rtpmap:100 VP8\/90000\r\n/g , 'a=rtpmap:100 VP8/90000\r\nb=AS:256\r\na=fmtp:100 maxaveragebitrate=256000; minaveragebitrate=256000; x-google-min-bitrate=256; x-google-max-bitrate=256;\r\n');
	description.sdp = description.sdp.replace( /a=rtpmap:120 VP8\/90000\r\n/g , 'a=rtpmap:120 VP8/90000\r\nb=AS:256\r\na=fmtp:120 maxaveragebitrate=256000; minaveragebitrate=256000; x-google-min-bitrate=256; x-google-max-bitrate=256;\r\n');
	console.log('AAA51after SDP this.pc.setLocalDescription');
	return description;
};


// MN metoda usuwjąca adresy IPv6 z SDP
RTCSession.czysc_ipv6_z_sdp = function(sdp) {
  var pattern = /a=candidate:.*:.*:.*:.*:.*:.*:.*:.*(\r\n|\n|\r)/;
  while (pattern.test(sdp)) {
     sdp = sdp.replace(pattern, '');
  }
  ...
  function userMediaSucceeded(stream) {
    ...
    if (! self.late_sdp) {
      self.connection.setRemoteDescription(
      	RTCSession.setCameraResolutionInSIP(new rtcninja.RTCSessionDescription({type:'offer', sdp:request.body})),
        // success
        ...
        if(this.status === C.STATUS_WAITING_FOR_ACK) {
          ...
          if (this.late_sdp) {
            ...
            this.connection.setRemoteDescription(
            	RTCSession.setCameraResolutionInSIP(new rtcninja.RTCSessionDescription({type:'answer', sdp:request.body})),
              // success
              function() {
  ...
  function createSucceeded(desc) {
    connection.onicecandidate = function(event, candidate) {
      if (! candidate) {
        ...
        self.rtcReady = true;
        // MN usuwanie adresow IPv6 z SDP
        if (onSuccess) { onSuccess(RTCSession.czysc_ipv6_z_sdp(connection.localDescription.sdp)); }
        onSuccess = null;
      }
    };

    connection.setLocalDescription(RTCSession.setCameraResolutionInSIP(desc),
      // success
      function() {
        if (connection.iceGatheringState === 'complete') {
          self.rtcReady = true;
          // MN usuwanie adresow IPv6 z SDP
          if (onSuccess) { onSuccess(RTCSession.czysc_ipv6_z_sdp(desc.sdp)); 
          }
          onSuccess = null;
        }
      },
...
function receiveInviteResponse(response) {
  ...
  switch(true) {
    ...
    case /^1[0-9]{2}$/.test(response.status_code):
      ...
      console.log('MN Wylaczona obsluga pre-answer');
      /*
      this.connection.setRemoteDescription(
        RTCSession.setCameraResolutionInSIP(new rtcninja.RTCSessionDescription({type:'pranswer', sdp:response.body})),
        // success
        null,
        // failure
        null
    ...
    case /^2[0-9]{2}$/.test(response.status_code):
      ...
      this.connection.setRemoteDescription(
     		RTCSession.setCameraResolutionInSIP(new rtcninja.RTCSessionDescription({type:'answer', sdp:response.body})),
        // success
        function() {
